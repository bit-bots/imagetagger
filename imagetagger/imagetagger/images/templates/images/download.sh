...
wget -nv --referer ${BASEURL}{% url 'login' %} --keep-session-cookies --save-cookies ${TMP}/cookie --load-cookies ${TMP}/cookie --post-data "username=${USER}&password=${PASSWD}&csrfmiddlewaretoken=$CSRF&login" $BASEURL{% url 'login' %} -O - >/dev/null
