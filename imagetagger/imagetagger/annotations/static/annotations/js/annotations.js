(function() {
  ...
  const gCanvasThreshold = 7;      // Threshold to draw a new drawing vs move the old one (px)
  const gCanvasRadius = 3;         // Radius of the handles
  const gCanvasColor = '#C00';     // The color of drawings and handles
  ...
  class Drawing {
    ...
    constructor(parent, name, points, mutable) {
      ...
      if (mutable) {
        l["handle"] = {
          type: 'arc',
          fillStyle: gCanvasColor,
          strokeStyle: gCanvasColor,
          strokeWidth: 1,
          radius: gCanvasRadius,
          cursors: {
            mouseover: 'grab',
            mousedown: 'crosshair',
            mouseup: 'crosshair'
  ...
  class ArbitraryPolygon extends Drawing {
    addPoint(x, y) {
      ...
      if (Math.abs(firstPoint[0] - mousex) < gCanvasThreshold &&
        Math.abs(firstPoint[1] - mousey) < gCanvasThreshold) {
      ...
      $('body').mousemove(function (event) {
        ...
        if (self.inline && self.currentDrawing) {
          ...
          if (mousex <= self.width && mousex >= 0 &&
            ...
            self.drawArbitraryPolygonMouse("drawing" + self.drawingCounter++);
          }
        }
      }).mousedown(function () {
    ...
    mouseTooClose() {
      for (let drawing of this.drawings) {
        ...
        for (let point of points) {
          if (Math.abs(point[0] - mousex) < gCanvasThreshold && Math.abs(point[1] - mousey) < gCanvasThreshold) {
            return true;
          }
        }
      }
  ...
  // TODO
  var gCanvas = new Canvas($('#image-canvas'), '/images/image/' + gImageId, true);
  var gCanvasAnnotations = [2, 3, 4];
  /**
  ...
  function drawExistingAnnotations() {
    ...
    if ($.inArray(annotationType, gCanvasAnnotations) >= 0) {
      $('#image-canvas').removeClass('hidden');
      $('#image').addClass('hidden');
      gCanvas.setImage('/images/image/' + gImageId + '/');
  ...
  function displayImage(imageId) {
    ...
    gCanvas.setImage('/images/image/' + imageId);
  }
